# TWF2026 みどころポータル — Claude.ai 引継ぎ資料 (5/13 夜時点 / 明日朝 5/14 メーカー本番Send 直前)

## ⚡ 最初に読むべきこと (新Claude向け)

あなたは Anthropic Claude (claude.ai)。柏原賢人さん (マツモト産業㈱京葉営業所課長、42歳、20年B2B営業 + 自学コーディング) の作業パートナー。

このプロジェクトは **TWF2026 (2026年6月12-13日 @ 幕張メッセNo.9ホール) の「みどころポータルサイト」** を、**主催店向け案内サイト + メーカー向け御礼/案内メール** の両軸で構築中。
...
- **5/12 夕方**: 7社ヒーロー背景画を gpt-image-1 で生成、074/147 は公式curlで実写画像取得、残5社はテキストオンリー (`product-card-textonly` 機構)。404.html + _redirects 導入。
- **5/12 夜**: 主催店宛メール送付完了 (TOP URL `https://twf2026-portal.pages.dev/` のみ告知)。
- **5/13 (水) 夜 緊急浄化セッション**: 主催店送付直後の最終品質チェックで多数の機密漏洩+表示品位問題を発見、19:00〜22:35 に **23コミット** で緊急対応 (詳細は § 5/13夜 緊急浄化セッション)。
- **5/14 (木) 朝 予定**: メーカー向け御礼/案内メール 128通を `twf2026_thanks_mailer.py` 経由で Outlook 下書き → 柏原手動 Send。

**残作業は明日朝のみ** (全146社の上部サマリー truncate 追加チェック + メーカー向けメール送付)。
...
主催店宛メール送付後 (5/12夜) の最終品質チェックで多数の機密漏洩+表示品位問題を発見、緊急対応。

### 23コミット要約

1. **Slug健全性 (17社)** `9b1fb70`: `kami-kou-tokoro→kobelco` / `uchida-megane→uchida-tokeiten` / `kisuuerujapan→kiswel-japan` 等、カナ音写→公式英字社名/正規読みに修正
2. **機密xlsx削除 (4件)** `1ef2dac` + `3753979`: ワキタ申込書 / アマダ HENNGE 案内PDF / ホータス特別価格表 / シャープMJ レイアウト
3. **引用ノイズクレンジング (8社12項目)** `8146f7b`: 長谷川 (案内メール全文引用) / エクシード (Q1-Q5全部) / 酸素アーク / ノビテック / 富士製砥 / ホタルクス / エクセル貿易 / ヨコタ工業の業務メール残骸を機械削除
4. **Q5一律廃止 (46社→0表示)** `27f76bb`: 配布資料・備考欄の文字回答を全社非表示 (添付セクション「📎 配布資料」で代替)
5. **上部サマリー再設計** `fa1c807` (削除しすぎ失敗) → `b14361d` (📌🎁🆕復活、📎件数だけ削除)
6. **エクシード (015)** `abb8eb7` + `a928717` + `1440383`: Q3「上記『■2.』」訂正 + 30MB PDF が Cloudflare 25MB上限超過でデプロイ失敗 → 外部 GCS URL 誘導の新フィールド `external_resources` 導入 + Q2 rewritten
7. **仕切率機密削除 (3社)** `3aa0025`: 日立産機 (104) / 京セラ (027) / テクノプラン (070) Q4 から仕切率消去 + スキャナー強化 (「貴社通常仕切り：定価の40%→37%」「通常仕切から29,000引き」「代理店様仕切り：＠41,500円」が portal で公開されていた重大事故)
8. **テクノプラン (070)** `ccc5f6e`: q2/q3 rewritten 追加 (truncate 解消)
9. **イチネンケミカル (011)** `bc62266`: truncate解消 + 配布資料2点追加
...
11. **工機HD+日動工業 配布資料追加 (5 PDF)** `1c8376a`: HiKOKIキャンペーン / トラッカープロ / CR36DSA + パノラマフラッシュ / ファイバースコープ

### 新規スクリプト・ツール

- **`scripts/_inspect_pricing_leak.py`** (`.gitignore` 対象、ワンショット): 仕切率機密検出スキャナー。パターン「`貴社通常仕切`」「`代理店[^\s]{0,4}仕切`」(間に「様」等の修飾語0-4字対応、テクノプラン漏れ事案から強化)「`定価のN%`」「`定価×0.X`」「`仕切.*N%.*対応`」「`代理店仕切`」「`販売店卸`」等
- **`scripts/_inspect_quote_pollution.py`** (`.gitignore` 対象): 引用ノイズスキャナー (`wrote:` / `From:` / `Sent:` / `柏原賢人` / `京葉2課` / `本メールにそのままご返信` / `【回答】欄にご記入` 等)
- **`scripts/_cleanse_quote_pollution.py`** (`.gitignore` 対象): 引用ノイズ機械クレンジング (区切り行以降カット、引用記号、定型句以降全カット)
...
- **`make_prompt(product, no)` の社別 override**: ニツコー 085 だけ「No text, no logos」抑制を解除し箱のテキスト生成を許可、他146社は従来通り
- **Q5 一律非表示テンプレ修正**: `q5_present = false` を `templates/maker_full.html.j2` で固定化、データJSON (`maker_details.json` / `maker_details_rewritten.json`) は無変更で将来の再有効化容易

### 学んだ教訓

- **Cloudflare Pages 個別ファイル 25MB上限**: エクシード 30MB PDF で初遭遇、`external_resources` 外部リンク誘導で対処。今後 25MB超の大型カタログ受領時は外部URL運用が標準
- **機密漏洩は機械検出だけでは取りこぼし発生**: 仕切率3件のうち2件 (京セラ + テクノプラン) が網羅スキャンの**拡張で発覚** (初回スキャンでは「代理店様仕切」を漏れ検出)、柏原目視と機械検出の組み合わせ必須
- **xlsx は原則 portal 公開不可ルール確立**: 4件削除の経験から (申込書 / 仕切表 / レイアウト図 / 案内メール PDF 化)。今後の attachments 受領時は `.xlsx` 拡張子は中身確認必須
- **AI画像生成は 4-5回 の試行錯誤前提**: 実物画像との一致は困難、メーカー要望は柏原目視で個別対応 (ニツコー 085 は5回 + 共通 suffix 専用 override で着地)
- **`.git/index.lock` の頻発**: 今夜だけで10回以上発生、memory `feedback_git_index_lock.md` の rule (`Get-Process git` + 0バイト確認後削除) を徹底
- **AskUserQuestion の使い方**: 柏原意図と私の解釈がズレた時 (ニツコー画像削除しすぎ事案) は最初の commit 前に確認すべき、無駄な往復を避けられる

### 明日朝 (5/14) の作業フロー (1時間予定)

1. **追加品質確認 (CC 30分)**
   - 全146社の上部サマリーで truncate 末尾「…」検出 (本夜4社で発覚した途切れ問題、他社にも残存可能性大)
   - 該当社に `q2_rewritten` / `q3_rewritten` / `q4_rewritten` 追加 (各140字以内)
2. **本番Send (15分)**
...
- portal slug 修正17社が本夜反映済 → URL は最新版で生成される (`kobelco` / `uchida-tokeiten` / `kiswel-japan` 等)
