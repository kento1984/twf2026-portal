# TWF2026 みどころポータル — Claude.ai 引継ぎ資料 (5/10 完了時点)
...
5/10 (日) は **sender 改修連携 + sync_attachments 修正で A 層 78 社まで拡大** + みどころ3選トピックページ + 当日特価チラシTOP配置で主催店送付資料完備。

**残作業は仕上げ系のみ** (製品画像21社リトライ / カテゴリ正規化 / 主催店宛メール文案)。
...
```
A層 78社: フル詳細 (Notion完全超え版) ✅ 5/10完成
B層 22社: パンフレット併載 (簡易) — 残はリッチ化対象 (5/12後でも可)
C層 48社: 情報準備中 (スケルトン) — TWF後に判断
```

### 5/9 → 5/10 の変動 (重要)
- **5/9 23:30 時点**: A=30 / B=39 / C=79
- **5/10 20:00 時点**: A=78 / B=22 / C=48
- **拡大の主要因**:
  1. `3e9fa92` sender (twf2026_sender) 側の has_answer 判定改修 (e603089) に追従、q1〜q4 のいずれか + reply_date あり、で answered と判定するよう緩和
  2. `1ce7969` sync_attachments の DEFAULTS パスを `\\flsv04\...` に正規化、旧 `\\fileserver\...` 撤廃で **PDF 取りこぼし 51 件解消**
  3. `2b5ef3c` 異体字 (CJK Radicals Supplement) 残存の正規化漏れを修正、信井→日立の alias 追加で 1 社救出
...
### 必須 (5/12 送付前)
1. **A層78社の目視チェック** — TOPカード画像 (シネマティック化) + 個別ページの Q1〜Q5 / ブランドカラー違和感確認
2. **製品画像 21社リトライ** — `_product_groups/group_2/3.json` を curl 方式で再実行
3. **カテゴリ正規化** — `data/makers.csv` の category 列、検索精度UP
4. **主催店宛メール文案** — URL案内 + 価値説明、5/12朝送付

### 任意 / 次フェーズ
5. **シャープ (049) URL 再判断** — 現 `https://smj.jp.sharp/bs/` vs 候補 `https://jp.sharp/`
6. **サイト全体トーン変更検討** — 黒基調 → 白基調 (柏原: 「ダーク基調暗くて変えようと思ってた」)
7. **B層 22社のリッチ化** — has_answer 拡張で残った B 層もリッチ化対象 (5/12後でも可)

### 5/9 想定の B層 39 社リッチ化計画について
**実質達成**。sender 改修連携 + sync_attachments 修正で B層 39 → 22 社に縮小、残った 22 社は本当にパンフのみで回答未着信。これらは TWF 後に判断。
...
### 短期 (5/11-5/12)
1. **A層78社の目視チェック** ← 最優先
   - 5/9 リッチ化済の 30 社 + 5/10 拡張分 48 社、Q1〜Q5 のリファインや brand color が未対応のまま
   - 拡張分 48 社の details_rewritten / brand 充実は時間が許せば追加
2. **製品画像21社リトライ** — group_2/3 を curl方式で再実行
3. **カテゴリ正規化** — data/makers.csv の category 列、3M=安全保護具、ナカトミ=冷却機器 等で検索精度UP
4. **主催店宛メール文案** — 5/12送付準備、URL案内 + 価値説明
5. **シャープ (049) URL再修正検討**
   - 現在: https://smj.jp.sharp/bs/ (CC判断)
   - 柏原さん指摘: https://jp.sharp/ (本体トップ) の方が客に親しみある可能性
   - 5/9夜は時間切れで未対応、柏原判断保留中
6. **(検討中) サイト全体トーン変更** — 黒基調 → 白基調 (柏原: 「ダーク基調暗くて変えようと思ってた」)

### 長期 (TWF後)
- B層22社の判断 — パンフのみ (本当に回答未着信) 社、TWF後に判断
- C層48社の判断 — 情報届いた社のみA層に昇格
...
$env:PYTHONUTF8=1
python scripts/build_html.py
# 出力: Maker pages rendered: A=78  B=22  C=48  total=148
```
...
5/10 までの作業はすべて完了 (A=78 / B=22 / C=48、みどころ3選 + 当日特価チラシ完備)。

【今日の最優先タスク】
1. 製品画像 21 社リトライ (group_2/3 を curl 方式で再実行)
2. カテゴリ正規化 (data/makers.csv の category 列を埋める)
3. A層78社の目視チェック (TOPカード画像 + 個別ページ Q1〜Q5)
...
4. **A 層 78 社の中身**: 5/9 リッチ化済の 30 社 (Q1〜Q5 リファイン + brand color + custom illust) と、5/10 拡張分 48 社 (元データのまま) で品質に濃淡。送付までに時間あれば拡張分の追加リファインも検討。
...
- [ ] 5/12 までに何をする? → 仕上げ (製品画像 / カテゴリ / 目視) + 主催店送付
- [ ] A/B/C 層の現状は? → A=78 / B=22 / C=48 (5/10時点、5/9から大幅拡大)
...
- [ ] 緊急時に何を見る? → HANDOFF.md (技術寄り) + HANDOFF_CLAUDEAI.md (戦略寄り) + 本番URL + git log
...
**5/12 目標まで残り約 36 時間、必須タスクは3つだけ (製品画像 / カテゴリ / 目視)。**
